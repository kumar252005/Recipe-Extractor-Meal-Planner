...
from app.scraper import ScrapeError, scrape_recipe_page
...
def extract_and_store_recipe(url: str, db: Session) -> Recipe:
    ...
    apply_recipe_payload(recipe, payload, scraped.text, raw_llm)
    db.commit()
    db.refresh(recipe)
    return recipe


def apply_recipe_payload(recipe: Recipe, payload: dict[str, Any], scraped_text: str, raw_llm_output: dict[str, Any]) -> None:
...
__all__ = ["ScrapeError", "build_meal_plan", "extract_and_store_recipe", "get_recipe", "list_recipes"]
